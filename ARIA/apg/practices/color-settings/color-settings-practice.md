...
....
      ...
      <section id="forced-colors">
        ...
        <p>Authors can use either <code>currentcolor</code> or <code>&lt;system-colors&gt;</code> CSS values for components to adapt to appropriate colors in the contrast theme.</p>
...
....
        ...
        <p>Using <code>&lt;system-colors&gt;</code> provides a more robust way for custom components to adapt to the appropriate system colors when users choose a contrast theme.  The best practice for using <code>&lt;system-colors&gt;</code> is in conjunction with the <code>forced-colors: active</code> media query, since using <code>&lt;system-colors&gt;</code> colors to exclusively specify component colors would result in both limiting the styling options of authors and content rendeirng varying widley between browsers.</p>

        <p>Authors need to use <code>&lt;system-colors&gt;</code> associated with the type of feature of the component.   Most commonly interactive components will be using the <code>buttonBorder</code>, <code>ButtonFace</code> and <code>ButtonText</code> values.  For more complex components to differentiate component features consider using <code>Highlight</code> or <code>ActiveText</code>.  See the HTML range slider (e.g. <code>input[type="range"])</code>) for how other colors than just button colors are used in styling the slider and thumb features.</p>
        ...
        <div class="table-wrap"><table class="data">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                The features of the thumb and the slider in the range can be differentiated by using <code>ActiveText</code> or <code>Highlight</code> values for the thumb.  They can also be used to enhance the identification of the current value by using them to fill in the left side of the rail to the thumb control.
              </td>
            </tr>
          </tbody>
        </table></div>
